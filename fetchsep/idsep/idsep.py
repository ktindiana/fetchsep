...
def apply_sliding_window(params, dates, fluxes_bg_in, fluxes, energy_bins, iteration=0,
    ...
    if is_final and params.idsep_resume:
        ...
        prev_dates, prev_fluxes = resume_fluxes(params, file_str=params.idsep_fname_sep, remove_sep=False)
        dates = prev_dates + dates
        for j in range(len(fluxes)):
            fluxes_high[j] = prev_fluxes[j] + fluxes_high[j]

        #Add in previous background with SEPs removed
        prev_dates, prev_fluxes_bg = resume_fluxes(params, file_str='fluxes_*.csv', remove_sep=True)
        for j in range(len(fluxes)):
            fluxes_bg[j] = prev_fluxes_bg[j] + fluxes_bg[j]
    ########### RESUME###############
...
def resume_fluxes(params, file_str='fluxes_*.csv', remove_sep=False,
    ...
    df = pd.read_csv(os.path.join(params.idsep_path, fname))
    ...
    #Remove the last point in the previous run file, because the flux read
    ...
    if trim_to_window:
        ...
        df = df.loc[df['Start Time'] >= firstdate]
    ...
    for key in bin_keys:
        ...
        if fname == '':
            sys.exit(f"RESUME: resume_fluxes: Cannot find file containing SEP times for {bin}. Exiting.")

        df_sep = pd.read_csv(os.path.join(params.idsep_path, fname), comment='#',
            header=None, names=['Start Time','End Time'])
...
def resume_derived_values(params, filename, energy_bins, trim_to_window=False):
    ...
    previous_bin_keys = cols + alt_energy_bins
    ...
    if trim_to_window:
        firstdate = lastdate - datetime.timedelta(days=params.sliding_win)
        firstdate = datetime.datetime(firstdate.year, firstdate.month, firstdate.day)
        df = df_mean.loc[df['Start Time'] >= firstdate]
        if df.empty:
...
def prepare_resume(params, energy_bins):
    ...
    flux_dates, fluxes = resume_fluxes(params, file_str='fluxes_*.csv', remove_sep=True, trim_to_window=True)

    #Previous mean background and sigma
    dates, means = resume_derived_values(params, params.idsep_fname_background, energy_bins, trim_to_window=True)
    dates, sigmas = resume_derived_values(params, params.idsep_fname_sigma, energy_bins, trim_to_window=True)

    if flux_dates != dates:
        sys.exit("RESUME: prepare_resume: The flux dates don't match the means, sigmas, thresholds dates. Exiting.")

    return dates, fluxes, means, sigmas
...
def resume_cut(params, dates, fluxes, energy_bins):
    ...
    prev_dates, prev_fluxes, prev_means, prev_sigmas = prepare_resume(params, energy_bins)
    #Prepend dates and fluxes with a sliding window's worth of fluxes from the previous run
    dates = prev_dates + dates
    nsigma = 4
    remove_above = []
    for i in range(len(fluxes)):
        fluxes[i] = prev_fluxes[i] + fluxes[i]
        remove_above.append(prev_means[i][-1] + nsigma*prev_sigmas[i][-1])
    ...
    return dates, fluxes_bg, fluxes_high
...
def run_idsep(str_startdate, str_enddate, experiment,
    ...
    if idsep_resume: #False if False or None
        lastdate = resume_fluxes(params, file_str='fluxes_*.csv', last_date_only=False)
        str_startdate = lastdate
    ...
    if params.idsep_resume:
        dates, fluxes_bg_init, fluxes_high_init = resume_cut(params, dates, fluxes, energy_bins)
    else:
    ...
    for iter in range(niter):
        ...
        if is_final and params.idsep_resume:
            ...
            dates = prev_dates + dates
            for j in range(len(fluxes)):
                fluxes[j] = prev_fluxes[j] + fluxes[j]
        ########### RESUME###############
