...
def sep_times_filename(params, startdate, enddate, energy_bin):
    ...
    fname =(f"{prename}_{zstdate}_{zenddate}_{energy_bin[0]}_to_{energy_bin[1]}.csv")
    return fname
...
def write_sep_dates(params, energy_bins, SEPstart, SEPend):
    ...
    for j in range(len(SEPstart)):
        fname = sep_times_filename(params, params.startdate, params.enddate, energy_bins[j])
...
def rough_cut(params, dates, fluxes, energy_bins):
    ...
    ave_dates, ave_fluxes, ave_sigma, threshold_dates, threshold =\
                ...
                params.idsep_nsigma, params.remove_above)
...
def set_resume_dates(params):
    """ Get start and end dates of previous idsep run """
    #Previous mean background and sigma
    df = pd.read_csv(os.path.join(params.idsep_resume_path, params.idsep_fname_background))
    df['dates'] = pd.to_datetime(df['dates'])
    
    #Get the last date in the files generated by the previous run of IDSEP
    firstdate = df.at[0,'dates']
    lastdate = df.at[len(df['dates'])-1,'dates']
    params.idsep_resume_firstdate = firstdate
    params.idsep_resume_lastdate = lastdate
    params.startdate = lastdate


def resume_fluxes(params, file_str='', remove_sep=False, trim_to_window=False):
    ...
    """
    directory = Path(params.idsep_resume_path)
    # Find all text files in this folder only
    if file_str == '':
        date_st = params.idsep_resume_firstdate.strftime("%Y%m%d")
        date_end = params.idsep_resume_lastdate.strftime("%Y%m%d")
        file_str = 'fluxes_*_' + date_st + '_' + date_end + '.csv'
    ...
    if fname == '':
        sys.exit(f"RESUME: resume_background_fluxes: Could not find {file_str} file in {params.idsep_resume_path}. Exiting.")
    df = pd.read_csv(fname)
    df['dates'] = pd.to_datetime(df['dates'])

    if params.startdate is None:
        sys.exit("resume_fluxes: Startdate is None. Get the resume start date by running set_resume_dates. Exiting.")
    ...
    if trim_to_window:
        firstdate = params.startdate - datetime.timedelta(days=params.sliding_win)
    ...
    for key in bin_keys:
        ...
        if '--' in key:
            bin = key.strip().split('--')
            energy_bin = [float(bin[0]), -1]
        else:
            bin = key.strip().split('-')
            energy_bin = [float(bin[0]), float(bin[1])]
 
        sepfname =  sep_times_filename(params, params.idsep_resume_firstdate, params.idsep_resume_lastdate, energy_bin)
        fname = ''
        for file in directory.glob(sepfname):
            fname = file

        if fname == '':
            sys.exit(f"RESUME: resume_fluxes: Cannot find file containing SEP times for {sepfname}. Exiting.")
        ...
        flux_bg, flux_sep = defbg.separate_with_dates(dates, flux_vals, SEPstart, SEPend, padding)
...
def resume_derived_values(params, filename, energy_bins, trim_to_window=False):
    ...
    """
    bgfilename = os.path.join(params.idsep_resume_path, filename)
    ...
    for bin in energy_bins:
        ...
        if bin_key not in previous_bin_keys:
            sys.exit(f"RESUME: prepare_resume: Energy bins do not match between new data {bin_key} and previous idsep results {previous_bin_keys} in {params.idsep_resume_path}. Exiting.")
...
def prepare_resume(params, energy_bins):
    ...
    df_fluxes, flux_dates, fluxes = resume_fluxes(params, remove_sep=True, trim_to_window=True)
...
def combine_resume_fluxes(params, dates, fluxes):
    ...
    prev_dates, prev_fluxes = resume_fluxes(params, remove_sep=False)
...
def back_up_resume_files(params):
    """ If resuming from a previous run in the same directory that will
        be used as output for the resume run, first copy the previous
        run files to .bak in case of a crash part way through.  
    ...
    for filenm in [params.idsep_fname_background, params.idsep_fname_sigma,
        params.idsep_fname_threshold, params.idsep_fname_kurtosis,
        params.idsep_fname_sep_bgsub, params.idsep_fname_sep]:
        
        filename = os.path.join(params.idsep_resume_path, filenm)
        shutil.copy2(filename, filename + '.bak')


def clean_up_resume_files(params):
    """ If resuming from a previous run in the same directory that will
        be used as output for the resume run, first copy the previous
        run files to .bak in case of a crash part way through.  
        
    """
    directory = Path(params.module_outpath)
    
    #Delete the added on part of the flux timeseries from the previous resume session
    rmstdate = params.idsep_resume_lastdate.strftime('%Y%m%d')
    rmenddate = params.enddate.strftime('%Y%m%d')
    file_str = f"*{rmstdate}_{rmenddate}*"
    for file in directory.glob(file_str):
        os.remove(file)
    ...
    for filenm in [params.idsep_fname_background, params.idsep_fname_sigma,
        ...
        filename = os.path.join(params.idsep_resume_path, filenm + '.bak')
        os.remove(filename)

    #Delete previous fluxes, SEPTimes, and HighPoints files
    #Delete files that were the starting point of this resume session
    #spanning the dates of the previous run
    rmstdate = dh.time_to_zulu(params.idsep_resume_firstdate).replace(':', '')
    rmenddate = dh.time_to_zulu(params.idsep_resume_lastdate).replace(':', '')
    file_str = f"*{rmstdate}_{rmenddate}*"
    for file in directory.glob(file_str):
        os.remove(file)
    ...
    #Delete files that were the starting point of this resume session
    #spanning the dates of the previous run
    rmstdate = params.idsep_resume_firstdate.strftime('%Y%m%d')
    rmenddate = params.idsep_resume_lastdate.strftime('%Y%m%d')
    file_str = f"*{rmstdate}_{rmenddate}*"
    for file in directory.glob(file_str):
        os.remove(file)
...
def run_idsep(str_startdate, str_enddate, experiment,
    ...
    if params.idsep_resume:
        #If resuming from the same path that contains the previous run,
        #back up the files from the previous run in case of a crash.
        back_up_resume_files(params)
    ...
    for iter in range(niter):
        ...
        if is_final and params.idsep_resume:
            #Add in previous original dates and fluxes
            dates, fluxes = combine_resume_fluxes(params, dates, fluxes)
            tools.write_fluxes(params, energy_bins, dates, fluxes)
    ...
    #Save config and parameters information for the run
    outputs = {
    ...
    stdtz = dh.time_to_zulu(params.startdate).replace(":","")
    enddtz = dh.time_to_zulu(params.enddate).replace(":","")
    outputs_fname = os.path.join(outputs["idsep_outpath"], f"{outputs['idsep_subdir']}.{stdtz}_{enddtz}_idsep_outputs.json")
    ccmc_json.write_json(outputs,outputs_fname)

    if params.idsep_resume:
        clean_up_resume_files(params)
