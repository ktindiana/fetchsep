...
# MAXIMUM ALLOWED RATIO OF AN INTERPOLATED/EXTRAPOLATED FLUX VALUE TO
# THE MAXIMUM ACTUALLY MEASURED FLUX AT THIS TIMESTEP. VALUES ABOVE
# THIS ARE SET TO NAN RATHER THAN DROPPED OR ZEROED, SO THEY REMAIN
# VISIBLE AS MISSING/BAD DATA RATHER THAN SILENTLY LOOKING LIKE A REAL
# ZERO FLUX. ADJUST BASED ON WHAT'S REASONABLE FOR YOUR SPECTRA.
EXTREME_VALUE_SANITY_FACTOR = 4
...
def data_processor(y, x, xn, extrapolate=True):
    ...
    result = loginterp(x1, y1, xn)

    # ZERO-OUT VALUES THAT ARE TOO MANY ORDERS OF MAGNITUDE ABOVE WHAT WAS
    # ACTUALLY MEASURED AT THIS TIMESTEP.
    result = np.where(result > max_measured * 10**EXTREME_VALUE_SANITY_FACTOR, 0.0, result)
...
def process_file(input_file, ebins, extrapolate, cores, plots=True):
    ...
    out_df = pd.DataFrame(interparray, columns=[f"{e:.6e}" for e in ebins])
    out_df.insert(0, time_col, df[time_col])
    out_df.to_csv(output_csv, index=False, float_format='%.6e', na_rep='NaN')
...
def main(input_file, extrapolate, ebin_file):
    
    cores = multiprocessing.cpu_count()-1
    # Process ebin files
    if ebin_file and os.path.exists(ebin_file):
        ebins = np.loadtxt(ebin_file)
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input-file", help="Path to fetchsep CSV output file.")
    parser.add_argument("--extrapolate", action="store_true", help="Enable Weibull extrapolation.")
    parser.add_argument("--ebin-file", type=str, default=None, help="File containing energy bins.")
    args = parser.parse_args()

    main(args.input_file, args.extrapolate, args.ebin_file)
